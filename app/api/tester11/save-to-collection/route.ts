...
export async function POST(request: NextRequest) {
  ...
  try {
    const { collectionId, imageUrl, prompt, analysis, instructions } = await request.json()
    ...
    // For tester11 images, we don't have traits or ordinal_number, so we use null
    ...
    const ordinal = (Array.isArray(result) ? result[0] : result) as any
    ...
    console.error('[Tester11 Save] Error:', error)
    return NextResponse.json(
