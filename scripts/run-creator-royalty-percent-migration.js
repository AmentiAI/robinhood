...
async function runMigration() {
  console.log('🚀 Running Migration 091: Add creator_royalty_percent...\n')
  ...
  try {
    console.log('  Adding creator_royalty_percent column...')
    await sql`
    ...
    const columns = await sql`
      ...
      AND column_name = 'creator_royalty_percent'
    `

    if (columns.length > 0) {
      const col = columns[0]
      console.log(`✅ Verified: creator_royalty_percent (${col.data_type}, nullable: ${col.is_nullable})`)
    }
